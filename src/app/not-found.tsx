...
export default function NotFound() {
  ...
  useEffect(() => {
    ...
    let mouse = { x: DISPLAY_WIDTH / 2, y: DISPLAY_HEIGHT / 2 };
    ...
    function initialize() {
      ...
      const header = document.querySelector('#fof>h1');
      if (header) {
    ...
    class Eye {
      ...
      x: number;
      y: number;
      iris: { x: number; y: number; size: number; };
      pupil: { width: number; height: number; };
      exposure: { top: number; bottom: number; current: number; target: number; };
      tiredness: number;
      isActive: boolean;
      ...
      update(mouse: { x: number, y: number }) { if (this.isActive === true) { this.render(mouse); } }
      ...
      render(mouse: { x: number, y: number }) {
        ...
        const eio = { x: (mouse.x / window.innerWidth) - 0.5, y: (mouse.y / window.innerHeight) - 0.5 };
  ...
  return (
    <div id="fof">
      ...
      <canvas></canvas>
      <h1>
        404 - Not Found. <Link href="/volunteer/dashboard">Go Home</Link>
      </h1>
    </div>
  );
}